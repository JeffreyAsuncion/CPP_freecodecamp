...
int main(){
    ...
    //      to which exponent should we elevate 2 to get 8? Log, by default computes the log
    ...
    std::cout << "Log ; to get 54.49, you would elevate e to the power of : " << std::log(54.49) << std::endl;
    ...
    // round . Halfway point are rounded away from 0. 2, 5 is rounded to 5 for example?
...
std::floor()        std::ceil()     std::abs()      std::cos()
        std::exp()          std::log()      std::pow()
std::sqrt()         std::round      std::sin()      std::tan()
